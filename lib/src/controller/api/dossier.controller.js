    ...
    async callbackOnlyoffice(req, res, next) {
        try {
            ...
            if (status === 2 || status === 3) {
                ...
                const newFileName = "ods-" + Math.random().toString(36).substring(2, 8) + '' + Math.random().toString(36).substring(2, 8) + path.extname(template.path);
                repository.update(template.id, {
                    "url": process.env.URL + "/files/" + newFileName,
                    "filename": newFileName,
                    "path": "files/" + newFileName,
                });
                fs.writeFileSync("files/" + newFileName, buffer);
